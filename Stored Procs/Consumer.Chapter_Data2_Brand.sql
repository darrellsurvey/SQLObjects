IF OBJECT_ID('Consumer.Chapter_Data2_Brand') IS NOT NULL
    DROP PROCEDURE [Consumer].[Chapter_Data2_Brand];
GO

CREATE procedure [Consumer].[Chapter_Data2_Brand]
...
SELECT [BRAND]
  ...
  where Brand = ''' + @Brand + ''' and p.YEAR between ' + cast(@Year - @YearsBack + 1 as varchar(4)) + ' and ' + cast(@Year as varchar(4)) + '
...
select ''New'' as DataType, ''YearsOfPlay'' as DemoGroup, LONG_PLAYED as DemoSubGroup,[YEAR], Brand,
COUNT(Brand) as BrandCount,
...
select ''New'' as DataType, ''CourseType'' as DemoGroup, COURSE as DemoSubGroup,[YEAR], Brand,
COUNT(Brand) as BrandCount,
...
select ''Total'' as DataType, ''YearsOfPlay'' as DemoGroup, LONG_PLAYED as DemoSubGroup,[YEAR], Brand,
COUNT(Brand) as BrandCount,
SUM(count(Brand)) over (partition by [YEAR], LONG_PLAYED) as TotalCount
from
(select year, brand, 
...
select ''Total'' as DataType, ''CourseType'' as DemoGroup, COURSE as DemoSubGroup,[YEAR], Brand,
COUNT(Brand) as BrandCount,
SUM(count(Brand)) over (partition by [YEAR], COURSE) as TotalCount
...
cte_AllBrands as (select distinct DataType, Brand,  [SequenceNum] as [year]
from cte_data d 
cross join (select distinct [SequenceNum] from [LKP].[Sequence] where [SequenceNum] between ' + cast(@Year - @YearsBack + 1 as varchar(4)) + ' and ' + cast(@Year as varchar(4)) + ') z
),
...
order by DataType desc, [YEAR] desc, sort asc, demosubgroup '
