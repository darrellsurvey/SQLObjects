...
SELECT 2014 as [YEAR], 1 AS [SEASON], 'b' as [BRAND], 'q' as [Series], 'r' as [Model], 'c' AS TotalType,
	1 as Total,
...
--coalesce([SERIES], [MODEL]) as SERIES,  
--case [SERIES] when NULL then [SERIES]
--else [MODEL] end as [MODEL],  


Set @sql = ' [YEAR], [SEASON], [BRAND], 
	SERIES, MODEL,	
	count(*) as Total,
...
if @NewNumber = 1 
		set @sqlNew = ' SELECT ''New'' AS TotalType, ' + @sql + ' and eq.YEARS < 2 group by [YEAR], SEASON, [BRAND], [SERIES], [MODEL] '
if @TotalNumber = 1 
		set @sqlTotal = ' SELECT ''Total'' AS TotalType, ' +  @sql + ' group by [YEAR],  SEASON, [BRAND], [SERIES], [MODEL] '
