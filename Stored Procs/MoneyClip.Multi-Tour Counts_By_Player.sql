IF OBJECT_ID('MoneyClip.Multi-Tour Counts_By_Player') IS NOT NULL
    DROP PROCEDURE [MoneyClip].[Multi-Tour Counts_By_Player];
GO

CREATE PROCEDURE [MoneyClip].[Multi-Tour Counts_By_Player]
(@FirstDay as date,
 @LastDay as date,
...
select 'a' as PLAYERNAME, 'b' as PGA, 'c' as [WEB.COM],
...
from (SELECT [TOUR],[FIRST DAY],[BRAND], [PLAYERNAME] FROM '
...
if CHARINDEX(',', @tourSort) > 0 
	SET @query = @query + '[PLAYERNAME] ASC'
else
	SET @query = @query + '[' + @tourSort + '] DESC, PLAYERNAME'
