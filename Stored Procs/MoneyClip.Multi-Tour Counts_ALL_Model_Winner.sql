IF OBJECT_ID('MoneyClip.Multi-Tour Counts_ALL_Model_Winner') IS NOT NULL
    DROP PROCEDURE [MoneyClip].[Multi-Tour Counts_ALL_Model_Winner];
GO

CREATE PROCEDURE [MoneyClip].[Multi-Tour Counts_ALL_Model_Winner]
(@FirstDay as date,
 @LastDay as date,
...
select 'Q' as Brand, 'a' as MODEL, 'b' as PGA, 'c' as [WEB.COM],
		'd' as NATIONWIDE, 'e' as CHAMPIONS, 'f' as LPGA,
		'g' as JGTO, 'h' as JLPGA, 'i' as CLPGA, 'j' as OTHER, 'k' AMATEUR, 'l' as ONEASIA,
		'n' as [BUY.COM], 'n' as JGA, 'o' AS LATINOAMERICA, 'p' AS CANADA
		

DECLARE @query AS varchar(MAX)
SET @query = 'select [BRAND] , [MODEL],
...
from (SELECT [TOUR],[FIRST DAY],[BRAND], Series as [MODEL], TournamentID, PlayerName FROM '
...
SET @query = @query + @EquipCase + @WhereCase +
  ...
  and [TOUR] in (' + @tour + ')) a
  left outer join [Money].[TourMoneyStats] m on  
		a.tournamentID = m.tournamentid and
		a.PlayerName = m.[Player Name]
  where m.[finish position] = 1
  group by [BRAND], [MODEL]
  order by '

if CHARINDEX(',', @tourSort) > 0 
	SET @query = @query + 'BRAND ASC, [MODEL] ASC'
else
	SET @query = @query + '[' + @tourSort + '] DESC, BRAND ASC, [MODEL]'
