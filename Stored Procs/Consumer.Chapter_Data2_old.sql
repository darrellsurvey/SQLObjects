IF OBJECT_ID('Consumer.Chapter_Data2_old') IS NOT NULL
    DROP PROCEDURE [Consumer].[Chapter_Data2_old];
GO

Create procedure [Consumer].[Chapter_Data2_old]
...
select *, RANK() over (Partition by DataType, [year], sort, DemoSubGroup order by BrandCount desc) as BrandRank
from cte_2
where BRAND not in (' + [DARRELL_MASTER].[Consumer].[BrandExceptionCase]() + ')
order by DataType desc, [YEAR] desc, sort asc, BrandRank asc '
