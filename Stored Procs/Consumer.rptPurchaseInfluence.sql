...
SELECT 2014 as [YEAR], 1 AS [SEASON], 'b' as [BRAND], 'c' AS PurchaseInfluenceSeries, 'd' AS PurchaseInfluence,
	1 as BrandInfluenceTotal, 1 as BrandTotal, 1 as BrandOverallTotal, 1 as BrandOverallRank, 1 as OverallTotal

Set @sql = ';with cte_original as (SELECT [YEAR], [SEASON], Brand, coalesce(INFLUENCE_SERIES, INFLUENCE) as PurchaseInfluenceSeries, INFLUENCE as PurchaseInfluence,
		count(*) as BrandInfluenceTotal, 
		...
		where p.COUNTRY = ''' + @Country + ''' and BRAND is not null and INFLUENCE is not null 
		...
		Set @sql = @sql + 'group by [YEAR], SEASON, [BRAND], INFLUENCE, INFLUENCE_SERIES),
		...
		SELECT a.year, a.season, a.brand, 
				PurchaseInfluenceSeries, PurchaseInfluence, BrandInfluenceTotal, BrandTotal, a.BrandOverallTotal, BrandOverallRank, OverallTotal
