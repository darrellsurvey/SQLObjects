IF OBJECT_ID('OptOut.Selection_LinkManager_Helper') IS NOT NULL
    DROP PROCEDURE [OptOut].[Selection_LinkManager_Helper];
GO
...
CREATE procedure [OptOut].[Selection_LinkManager_Helper]
...
if ((select count(*) from [OptOut].[LinkManager] where playername = @playername and feedname = @feedname) = 0)
	if (@feedname = 'whatsinmybag')
		...
		INSERT INTO [OptOut].[LinkManager] (feedname, equipment, brand, model, clubnumber, link_url, addedon, addedby)
		SELECT DISTINCT @feedname, sd.equipment, sd.brand, sd.model, sd.clubnumber, sd.default_link_url, GETDATE(), '[OptOut].[Selection_LinkManager_Helper]'
		FROM #SelectionData sd
		LEFT JOIN [OptOut].[LinkManager] lm on lm.feedname = @feedname and coalesce(sd.equipment, '') = coalesce(lm.Equipment, '') and sd.brand = lm.brand and coalesce(sd.model, '') = coalesce(lm.model, '') and coalesce(sd.clubnumber, '') = coalesce(lm.clubnumber, '')
		where lm.link_id is null and sd.default_link_url is not null and sd.default_link_url <> ''
