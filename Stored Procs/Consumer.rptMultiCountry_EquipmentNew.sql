IF OBJECT_ID('Consumer.rptMultiCountry_EquipmentNew') IS NOT NULL
    DROP PROCEDURE [Consumer].[rptMultiCountry_EquipmentNew];
GO

CREATE procedure [Consumer].[rptMultiCountry_EquipmentNew]
...
set nocount on;
	...
	if @get_column_names_only = 1 
		select 1 as [year], 'b' as [Country], 1 as Sort, 'c' as Equip, 3 as CountryNew
...
SELECT distinct p.[year],p.COUNTRY, 1 as sort, ''Iron'' as Equip
		,sum(case when years < 2 then 1 else 0 end) * 100.0 / Count(*) as CountryNew
