IF OBJECT_ID('CR_Reports.Iron_Wood_Shaft') IS NOT NULL
    DROP PROCEDURE [CR_Reports].[Iron_Wood_Shaft];
GO
...
-- =============================================
CREATE PROCEDURE [CR_Reports].[Iron_Wood_Shaft]
...
INNER JOIN
(
SELECT [Name] AS PLAYERNAME, [Shaft Club Code] AS IRONCLUBCODE, DCLUBCODE, ISNULL(d.[Mfgr Abbrev], d.[Mfgr Descr]) AS IRONSHAFTMFGR, DMFGRCODE,
...
ISNULL(c.[Matl Abbrev], c.[Matl Descr]) AS IRONSHAFTMATL, DMATERIAL, [PKey], [Shaft Equip Type]
FROM Player_Master.[Shaft Detail]
LEFT OUTER JOIN LKP.[Manufacturer Codes and Desc] a ON [Shaft Brand Code] = a.[Mfgr Code]
LEFT OUTER JOIN LKP.[Model Codes and Descr] b ON [Shaft Model Code] = b.[Model Code]
LEFT OUTER JOIN LKP.[Material Codes and Descript] c ON [Shaft Mat'l Code] = c.[Matl Code]
LEFT OUTER JOIN LKP.[Manufacturer Codes and Desc] d ON [Shaft Mfgr Code] = d.[Mfgr Code]
...
ISNULL(c.[Matl Abbrev], c.[Matl Descr]) AS WOODSHAFTMATL, DMATERIAL,
[PKey]
FROM Player_Master.[Shaft Detail]
LEFT OUTER JOIN LKP.[Manufacturer Codes and Desc] a ON [Shaft Brand Code] = a.[Mfgr Code]
LEFT OUTER JOIN LKP.[Model Codes and Descr] b ON [Shaft Model Code] = b.[Model Code]
...
IRONCLUBCODE AS IRONSHAFTCLUBCODE, IRONSHAFTTABLE.DCLUBCODE, IRONSHAFTMFGR, IRONSHAFTTABLE.DMFGRCODE, IRONSHAFTBRAND, IRONSHAFTTABLE.DBRANDCODE, IRONSHAFTMODEL, IRONSHAFTTABLE.DMODELCODE, IRONSHAFTMATL, IRONSHAFTTABLE.DMATERIAL,
WOODCLUBCODE AS WOODSHAFTCLUBCODE, WOODSHAFTTABLE.DCLUBCODE, WOODSHAFTMFGR, WOODSHAFTTABLE.DMFGRCODE, WOODSHAFTBRAND, WOODSHAFTTABLE.DBRANDCODE, WOODSHAFTMODEL, WOODSHAFTTABLE.DMODELCODE, WOODSHAFTMATL, WOODSHAFTTABLE.DMATERIAL
