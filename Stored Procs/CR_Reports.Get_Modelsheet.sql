...
CREATE PROCEDURE [CR_Reports].[Get_Modelsheet]
...
(SELECT COUNT([Iron Brand Code]) FROM [Player_Master].[Iron Detail] WHERE [Survey ID] = @SID AND [First Day] = @FIRSTDAY) AS [TOTAL]
  ...
  WHERE [Survey ID]= @SID AND [First Day] = @FIRSTDAY and ISSET = 1
...
(SELECT COUNT([Grip Brand Code]) FROM [Player_Master].[Grip Detail] WHERE [Survey ID] = @SID AND [First Day] = @FIRSTDAY AND [Grip Equip Type] = 'IRON') AS [ TOTAL]
  ...
  WHERE [Survey ID]= @SID AND [First Day] = @FIRSTDAY AND [Grip Equip Type] = 'IRON'
