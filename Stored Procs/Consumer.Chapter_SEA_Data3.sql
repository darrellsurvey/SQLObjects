IF OBJECT_ID('Consumer.Chapter_SEA_Data3') IS NOT NULL
    DROP PROCEDURE [Consumer].[Chapter_SEA_Data3];
GO

CREATE procedure [Consumer].[Chapter_SEA_Data3]
...
--exec [Consumer].[Chapter_SEA_Data3] 'Ball', 'Thailand', 2019,2,5

declare @PrevYear int
select @PrevYear = [Year] from (select top 1 [Year] from (select distinct [year] from Consumer.PlayerProfile where COUNTRY in ('Thailand')  and SEASON = 2 and [year] < @Year) a order by [year] desc) b
...
SELECT [BRAND]
  ...
  where p.YEAR in (' + cast(@PrevYear as varchar(4)) + ', ' + cast(@Year as varchar(4)) + ')
...
select ''New'' as DataType, 6 as sort, ''YearsOfPlay'' as DemoGroup, LONG_PLAYED as DemoSubGroup,[YEAR], Brand,
COUNT(Brand) as BrandCount,
...
select ''New'' as DataType, 7 as sort, ''CourseType'' as DemoGroup, COURSE as DemoSubGroup,[YEAR], Brand,
COUNT(Brand) as BrandCount,
...
select ''Total'' as DataType, 6 as sort, ''YearsOfPlay'' as DemoGroup, LONG_PLAYED as DemoSubGroup,[YEAR], Brand,
COUNT(Brand) as BrandCount,
SUM(count(Brand)) over (partition by [YEAR], LONG_PLAYED) as TotalCount
...
select ''Total'' as DataType, 7 as sort, ''CourseType'' as DemoGroup, COURSE as DemoSubGroup,[YEAR], Brand,
COUNT(Brand) as BrandCount,
SUM(count(Brand)) over (partition by [YEAR], COURSE) as TotalCount
...
case when demogroup = ''Total'' and DemoSubGroup = 1 then ''Total'' 
	 ...
	 when demogroup = ''YearsOfPlay'' and DemoSubGroup = 1 then ''1 & Under'' 
	 when demogroup = ''YearsOfPlay'' and DemoSubGroup = 2 then ''2 - 5 years'' 
	 when demogroup = ''YearsOfPlay'' and DemoSubGroup = 3 then ''6 - 10 years'' 
	 when demogroup = ''YearsOfPlay'' and DemoSubGroup = 4 then ''11 - 15 years'' 
	 when demogroup = ''YearsOfPlay'' and DemoSubGroup = 5 then ''16 - 20 years''
	 when demogroup = ''YearsOfPlay'' and DemoSubGroup = 6 then ''21 years & up''
	 when demogroup = ''CourseType'' and DemoSubGroup = 1 then ''Golf Course'' 
	 when demogroup = ''CourseType'' and DemoSubGroup = 2 then ''Range'' 
...
cte_3 as (
select * from cte_2
pivot (sum(BrandPercent) for [Year] in ([' + cast(@PrevYear as varchar(4)) + '],[' + cast(@Year as varchar(4)) +'])) p)


select DataType, sort, demogroup, DemoSubGroup,DemoSubGroupText, Brand,
		[' + cast(@PrevYear as varchar(4)) + '] as PrevYear, 
		[' + cast(@Year as varchar(4)) +'] as NowYear, 
		Row_Number() over (Partition by DataType, sort, DemoSubGroup order by [' + cast(@Year as varchar(4)) +'] desc) as BrandRank
from cte_3
