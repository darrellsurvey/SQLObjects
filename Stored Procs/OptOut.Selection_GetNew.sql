IF OBJECT_ID('OptOut.Selection_GetNew') IS NOT NULL
    DROP PROCEDURE [OptOut].[Selection_GetNew];
GO

CREATE procedure [OptOut].[Selection_GetNew]
@user varchar(100),
@PlayerName varchar(100), 
@Season integer,
@skip_recalculate bit = 0,
@feedname_for_links varchar(100) = 'default'

as
begin
SET NOCOUNT ON
--  exec [OptOut].[Selection_GetNew] 'alex', 'MORIKAWA, COLLIN', 2025
--  exec [OptOut].[Selection_GetNew] 'alex', 'HOVLAND, VIKTOR', 2025

if (@skip_recalculate = 0)
BEGIN
...
a.Brand, a.Equipment, a.Model, Shaft, Grip, a.ClubNumber, Loft,
--old ShopifyId, default_link_url + '?wimb=' + ShopifyCollectionHandle as default_link_url, player_link_url + '?wimb=' + ShopifyCollectionHandle as player_link_url, coalesce(SetByUserId, 0) as SetByUserId, coalesce(SetBy, 'U') as SetBy, a.AddedOn 
-- populate date ShopifyId, default_link_url, player_link_url, coalesce(SetByUserId, 0) as SetByUserId, coalesce(SetBy, 'U') as SetBy, a.AddedOn 
ShopifyId, storelinks.link_url + '?wimb=' + ShopifyCollectionHandle as default_link_url, mystorelinks.link_url + '?wimb=' + ShopifyCollectionHandle as player_link_url, coalesce(SetByUserId, 0) as SetByUserId, coalesce(SetBy, 'U') as SetBy, a.AddedOn 
...
LEFT OUTER JOIN [OptOut].[LinkManager] mystorelinks ON mystorelinks.feedname = 'whatsinmybag' AND a.playername = mystorelinks.playername AND a.Brand = mystorelinks.Brand AND COALESCE(a.model, '') = COALESCE(mystorelinks.model, '') AND COALESCE(a.Equipment, '') = COALESCE(mystorelinks.Equipment, '') AND COALESCE(a.ClubNumber, '') = COALESCE(mystorelinks.ClubNumber, '')
--LEFT OUTER JOIN [OptOut].[LinkManager] storelinks ON storelinks.feedname <> 'whatsinmybag' and a.playername = coalesce(storelinks.playername, a.playername) AND a.Brand = storelinks.Brand AND COALESCE(a.model, '') = COALESCE(storelinks.model, '') AND COALESCE(a.Equipment, '') = COALESCE(storelinks.Equipment, '') AND COALESCE(a.ClubNumber, '') = COALESCE(storelinks.ClubNumber, '')
--LEFT OUTER JOIN (select Distinct PlayerName, case when feedname <> 'whatsinmybag' then 'misc' else 'whatsinmybag' end as feedname,[Equipment],[Brand],[Model],[ClubNumber],[link_url] from [OptOut].[LinkManager]) storelinks ON storelinks.feedname <> 'whatsinmybag' and a.playername = coalesce(storelinks.playername, a.playername) AND a.Brand = storelinks.Brand AND COALESCE(a.model, '') = COALESCE(storelinks.model, '') AND COALESCE(a.Equipment, '') = COALESCE(storelinks.Equipment, '') AND COALESCE(a.ClubNumber, '') = COALESCE(storelinks.ClubNumber, '')
LEFT OUTER JOIN [OptOut].[LinkManager] storelinks ON storelinks.feedname = @feedname_for_links and a.playername = coalesce(storelinks.playername, a.playername) AND a.Brand = storelinks.Brand AND COALESCE(a.model, '') = COALESCE(storelinks.model, '') AND COALESCE(a.Equipment, '') = COALESCE(storelinks.Equipment, '') AND COALESCE(a.ClubNumber, '') = COALESCE(storelinks.ClubNumber, '')

where a.PLAYERNAME = @PlayerName and Season > 0
order by case when selectionlevel < 2 then selectionlevel else 999 end, a.Brand, a.Equipment, a.ClubNumber, a.model, a.Loft
