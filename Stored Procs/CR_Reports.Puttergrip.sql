IF OBJECT_ID('CR_Reports.Puttergrip') IS NOT NULL
    DROP PROCEDURE [CR_Reports].[Puttergrip];
GO
...
-- =============================================
CREATE PROCEDURE [CR_Reports].[Puttergrip]
...
ISNULL(c."Mfgr Abbrev", c."Mfgr Descr") AS BRAND, DBRANDCODE, ISNULL(d."Model Abbrev", d."Model Descr") AS MODEL, DMODELCODE, ISNULL(f."Matl Abbrev", f."Matl Descr") AS MATL, DMATERIAL
FROM (

SELECT "Name", "Survey ID", "First Day" FROM Player_Master.[Putter Detail] WHERE "First Day" = @FIRSTDAY AND "Survey ID" = @SID GROUP BY "Name", "Survey ID", "First Day") a 
LEFT OUTER JOIN (SELECT * FROM Player_Master.[Grip Detail] WHERE "First Day" = @FIRSTDAY AND "Survey ID" = @SID AND "Grip Equip Type" = 'PUTT') b
ON a."Name" = b."Name"
