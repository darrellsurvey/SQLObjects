IF OBJECT_ID('Consumer.Chapter_Data_Model') IS NOT NULL
    DROP PROCEDURE [Consumer].[Chapter_Data_Model];
GO

CREATE procedure [Consumer].[Chapter_Data_Model]
...
--exec [Consumer].[Chapter_Data_Model] 'Wood - Driver', 'Thailand', 2019, 2, 'Taylormade'
...
set @sqlMaintext = '; with cte_PP_Total as (SELECT Brand, COALESCE(SERIES, MODEL) as Model, '
...
if not (@Equipment = 'Ball' or @Equipment = 'Headgear' or @Equipment = 'Shirt') 
	set @sqlMaintext = @sqlMaintext + ' cte_PP_New as (SELECT Brand, COALESCE(SERIES, MODEL) as Model, ' + @text1 +
		     ' and YEARS < 2), '
