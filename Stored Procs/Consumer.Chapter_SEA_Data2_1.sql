IF OBJECT_ID('Consumer.Chapter_SEA_Data2_1') IS NOT NULL
    DROP PROCEDURE [Consumer].[Chapter_SEA_Data2_1];
GO

CREATE procedure [Consumer].[Chapter_SEA_Data2_1]
...
FROM (select year, COUNTRY, brand, 
...
(select year, COUNTRY, brand, 
	 case when HANDICAP < 11 then 1
	 when HANDICAP between 11 and 20 then 2
	 when HANDICAP > 20 then 3
end as handicap
