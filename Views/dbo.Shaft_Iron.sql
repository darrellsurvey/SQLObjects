IF OBJECT_ID('dbo.Shaft_Iron') IS NOT NULL
    DROP VIEW [dbo].[Shaft_Iron];
GO

CREATE VIEW dbo.Shaft_Iron
AS
SELECT        TOP (100) PERCENT Player_Master.TOURNAMENTS_TABLE.SID, Player_Master.TOURNAMENTS_TABLE.TYPE AS TOUR, 
                         ...
                         Player_Master.[Iron Detail].ISSET AS SPEC_FLAG, Player_Master.[Shaft Detail].[Shaft Club Code] AS [CLUB NUMBER], 
...
FROM            LKP.[Manufacturer Codes and Desc] INNER JOIN
                         ...
                         Player_Master.[Iron Detail] ON Player_Master.[Shaft Detail].Name = Player_Master.[Iron Detail].Name AND 
                         Player_Master.[Shaft Detail].[Survey ID] = Player_Master.[Iron Detail].[Survey ID] AND 
                         Player_Master.[Shaft Detail].[First Day] = Player_Master.[Iron Detail].[First Day] AND 
                         Player_Master.[Shaft Detail].PKey = Player_Master.[Iron Detail].PKey INNER JOIN
...
WHERE        (Player_Master.[Shaft Detail].[Shaft Equip Type] = N'IRON')
