...
SELECT     TOP (100) PERCENT Player_Master.TOURNAMENTS_TABLE.SID, Player_Master.TOURNAMENTS_TABLE.TYPE AS TOUR, 
                      ...
                      Player_Master.[Wood Detail].ISDRIVER AS SPEC_FLAG, Player_Master.[Grip Detail].[Grip Club Code] AS [CLUB NUMBER], 
...
FROM         LKP.[Manufacturer Codes and Desc] INNER JOIN
                      ...
                      Player_Master.[Wood Detail] ON Player_Master.[Grip Detail].PKey = Player_Master.[Wood Detail].PKey AND 
                      Player_Master.[Grip Detail].Name = Player_Master.[Wood Detail].Name AND Player_Master.[Grip Detail].[Survey ID] = Player_Master.[Wood Detail].[Survey ID] AND 
                      Player_Master.[Grip Detail].[First Day] = Player_Master.[Wood Detail].[First Day]
WHERE     (Player_Master.[Grip Detail].[Grip Equip Type] = N'WOOD')
