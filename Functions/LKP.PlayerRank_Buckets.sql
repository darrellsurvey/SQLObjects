IF OBJECT_ID('LKP.PlayerRank_Buckets') IS NOT NULL
    DROP FUNCTION [LKP].[PlayerRank_Buckets];
GO
...
CREATE FUNCTION [LKP].[PlayerRank_Buckets]
(
...
;with CTE_Bucket as (
		...
		where t.Type = @Tour and 
				year(t.[FIRST DAY]) between @year-2 and @Year and
				m.[player name] = @PlayerName)

select @Return = sum(PointsWon * TournamentRatio * YearRatio) from CTE_Bucket   
